#setting the parameters for drawing samples using normal distribution
sample=1000
n=20
mean = 40
sd=5
...
mat <- matrix(rnorm(n, mean = mean, sd = sd),ncol=n)

#filling up of above matrix
for (i in 2:1000) {
  mat1 <- matrix(rnorm(n, mean = mean, sd = sd),ncol=n) 
  mat<-rbind(mat, mat1)
}
